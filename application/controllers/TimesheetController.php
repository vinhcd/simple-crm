...
class TimesheetController extends MyZendControllerAction{
...
$bodytext="
Staff: ".$admin->staff_id."/".$admin->staff_name."
Comment content:
".$req["comment"]."
Date: ".$req["date"]."
URL:
...
";		
		$this->backgroundPost("http://timesheet.neoscorp.vn/sendmail.php?subject=".urlencode('Comment from '.$admin->staff_name)."&body=".urlencode($bodytext));
		
		//file("http://timesheet.neoscorp.vn/sendmail.php?subject=".urlencode('Comment from '.$admin->staff_name)."&body=".urlencode($bodytext));
/*		
...
public function editAction(){
	...
	if (isset($req["ac"]) && $req["ac"]==1){
		...
		if ($db->query_update("time_sheet", $data, "id=".$req["id"])===false){
			$db->close();
			die("err");
		}	
		...
		$annual = new Annual();	
		$annual->updateAnnual($req["staff_id"], $db);	
...
//echo $start_time;die;
		...
		if ($db->query_insert("time_sheet", $data)===false){
			$db->close();
			die("err");
		}		
...
//		echo $date_from;
		...
		while ($rows = $db->fetch_array($rs)) {	
			...
			$sql="select * from ngayphep where staff_id='".$rows["id"]."' and month_year='".date("Y-n", $date_from)."'";
			$ngayphep_one= $db->query_first($sql);
			$o_smarty->assign("ngayphep_one", $ngayphep_one);
		
		
			for ($j=$date_from; $j<=$date_to; $j+=86400) {  
				//echo date("Y-m-d", $j)."<br>";
				if ($common->isThisDayAWeekend(date("Y/m/d", $j))) $woking_time[$i]['color']="red";
				...
				if (!empty($national_days_one)) $woking_time[$i]['color']="holiday";
		...
		$o_smarty->assign("total_mh", $total_mh);
		$o_smarty->assign("ot1", $ot1);
		$o_smarty->assign("ot2", $ot2);
		$o_smarty->assign("ot3", $ot3);
		$o_smarty->assign("ot4", $ot4);
		...
		while ($rows = $db->fetch_array($rs)) {	
			...
			for ($j=$date_from; $j<=$date_to; $j+=86400) {  
				...
				if (!empty($time_sheet_one["check_in"]) && !empty($time_sheet_one["check_out"])){
					...
					$sheet->setCellValue('J'.$i, $wt[1]);
					$sheet->setCellValue('K'.$i, $wt[2]);
					$sheet->setCellValue('L'.$i, $wt[3]);
					$sheet->setCellValue('M'.$i, $wt[4]);					
				...
				//tinh gio lam thieu
				if (!$common->isThisDayAWeekend(date("Y/m/d", $j)) && empty($national_days_one))
				 	$sheet->setCellValue('I'.$i, 8-$work_hours);
  ...
  public function uploadAction(){
	...
	$sql="delete from time_sheet_log";
	...
	for ($i=1; $i<count($data); $i++){
		...
		if (empty($time_sheet_org) && !empty($staff_one)){
			...
			if ($db->query_insert("time_sheet_log", $datav)===false){
				$db->query("rollback");
				$db->close();
				die("err1");
			}			
	...
	$sql="SELECT staff_id, min(date_time) as start_time, max(date_time) as finish_time FROM `time_sheet_log` GROUP BY staff_id, DATE_FORMAT(date_time, \"%Y-%m-%d\")";
	$rs = $db->query($sql);	
	//$i=0;
	while ($rows = $db->fetch_array($rs)){

		$sql="select * from staff where id_timesheet_machine='".trim($rows["staff_id"])."'";
		$staff_one= $db->query_first($sql);
			
		$datav=array();
		$datav["staff_id"]=$staff_one['id'];
		$datav["check_in"]=$rows["start_time"];
		$datav["check_out"]=$rows["finish_time"];
		$datav["check_in_org"]=$rows["start_time"];
		$datav["check_out_org"]=$rows["finish_time"];				
		$datav["create_date"]=time();
		$datav["last_update"]=time();
		$datav["last_update_staff_id"]=trim($rows["staff_id"]);
	...
	*/
	...
	public function updateAnnualAction(){
		...
		$sql="select * from ngayphep where staff_id='".$req["staff_id"]."' and month_year='".$req["month_year"]."'";
		...
		if (empty($ngayphep_one2)){
			$data=array();
			$data["staff_id"]=$req["staff_id"];
			$data["month_year"]=$req["month_year"];
			if (empty($req["dieuchinhphep"]))
				$data["dieuchinhphep"]="null";
			else
				$data["dieuchinhphep"]=$req["dieuchinhphep"];
			$data["comment"]=$req["comment"];
			...
			if ($db->query_update("ngayphep", $data, "staff_id='".$req["staff_id"]."' and month_year='".$req["month_year"]."'")===false){
				$db->query("rollback");
				$db->close();
				die("err1");
			}				
